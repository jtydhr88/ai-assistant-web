...
function removeColor(basePrompt) {
  ...
  const cleanedTags = promptList.filter(
    (tag) => !colorList.some((color) => tag.toLowerCase().includes(color.toLowerCase()))
  );
...
function executePrompt(executeTags, basePrompt) {
  ...
  const promptList = basePrompt.split(", ");
  const filteredTags = promptList.filter((tag) => !executeTags.includes(tag));
